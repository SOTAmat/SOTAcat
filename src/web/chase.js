...
function updateRefreshButtonLabel() {
    ...
    if (ChaseState.autoRefreshEnabled) {
        ...
        Log.debug("Chase")("Button state: Auto-refresh?");
 ...
 * Theme-aware: light returns warm pink → red; dark returns dim warm → muted red.
 * Fresh spots (< 5 min) return 'transparent'.
 */
...
function buildChaseRow(spot, isMySpot) {
    ...
    // Age-based coloring: fresh spots are neutral, older spots turn warm/red (theme-aware)
    ...
    if (refreshButton) {
        refreshButton.addEventListener("click", () => {
            ...
            if (ChaseState.autoRefreshEnabled) {
                ...
                // Data is already fresh from the refresh that triggered this prompt
                startAutoRefresh();
            } else {
                // Normal manual refresh
