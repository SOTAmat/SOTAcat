...
esp_err_t handler_power_put (httpd_req_t * req) {
    ...
    {
        const std::lock_guard<Lockable> lock (kxRadio);
        // NOTE: Do not force zero first; this creates a brief 0 W on the KX display
        // We now directly request the desired power and verify.

        if (!desired_power)
            REPLY_WITH_SUCCESS();  // wanted zero - we're done

        ESP_LOGI (TAG8, "power_put: setting PC to %ld", desired_power);
        ...
        if (power != desired_power)
            ESP_LOGI (TAG8, "power_put: requested power '%s', acquired only %ld", param_value, power);
        if (power == 0)
