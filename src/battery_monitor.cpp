...
#include <esp_log.h>
static const char *TAG8 = "sc:battery_monitor";
...
float vbat_analog = 0;
float vpct_analog = 0;
float vbat_digital = 0;
float vpct_digital = 0;
    ...
    while(1){
        ...
        if(max17260_detected){
            ...
            if(!(cnt % REPORTING_TIME_SEC)){
                ESP_LOGI(TAG8, "Battery: %4.2fV %4.1f%% %5.1fmA %s", vbat_digital,
                                                                     vpct_digital,
                                                                     bat_info.current_average,
                                                                     (bat_info.charging ? "charging" : "discharging"));
            }
        }else{
            if(!(cnt % REPORTING_TIME_SEC)){
