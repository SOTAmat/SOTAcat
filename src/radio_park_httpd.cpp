...
void radio_park_init (httpd_handle_t server) {
    ...
    s_server = server;
    ESP_LOGI (TAG8, "radio park initialised (max %d, get %u ms, set %u ms)", RADIO_PARK_MAX, (unsigned)RADIO_PARK_GET_WAIT_MS, (unsigned)RADIO_PARK_SET_WAIT_MS);
}
...
bool radio_park_request (httpd_req_t * req, RadioParkKind kind, uint32_t gen, uint32_t wait_ms, radio_park_completer_t completer) {
...
#ifdef SOTACAT_SOAK_DIAG
    {
        ...
        if (t - last_diag > 60'000'000) {
            last_diag = t;
            ESP_LOGI (TAG8, "DIAG httpd stack min-free=%u B; parked=%d", (unsigned)uxTaskGetStackHighWaterMark (NULL) * sizeof (StackType_t), s_table.count());
        }
    }
#endif
